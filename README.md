...
这是源材料差分导入工具，不运行测试或编译。它会重新生成 smali 补丁、资源差分和指纹清单，并调用 `tools/patch_candidate_scroll.py` 重新应用九键拼音跟手滚动改动。仅重建这项功能时可运行 `python tools/patch_candidate_scroll.py --origin /path/to/origin --mod /path/to/mod`。更换目标版本后，需要重新确认差异含义与设备兼容性；生成器遇到方法或指令结构不符会停止。
